...
impl MapIterator {
    ...
    pub(crate) fn next(this: &Value, _args: &[Value], ctx: &mut Context) -> Result<Value> {
        if let Value::Object(ref object) = this {
            ...
            if let Some(map_iterator) = object.as_map_iterator_mut() {
                ...
                let len = {
                    if let Some(map) = map_iterator.map.as_object().unwrap().as_map_ref() {
                        map.len() as u32
                    } else {
                        unimplemented!()
                    }
                ...
                dbg!(len);
                dbg!(map_iterator.next_index);

                // let len = map_iterator
                //     .map
                //     .as_number()
                //     .ok_or_else(|| ctx.construct_type_error("Not an map"))?
                //     as u32;
                ...
                match map_iterator.kind {
                    ...
                    MapIterationKind::Value => {
                        let obj = map_iterator.map.as_object().unwrap();
                ...
                ctx.throw_type_error("`this` is not a MapIterator")
            }
        } else {
            ctx.throw_type_error("`this` is not a MapIterator")
        }
    ...
    pub(crate) fn create_prototype(ctx: &mut Context, iterator_prototype: Value) -> Value {
        ...
        let to_string_tag_property =
            Property::data_descriptor(Value::string("Map Iterator"), Attribute::CONFIGURABLE);
